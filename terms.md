...
2.  The _Proposal_ may identify:
    ...
    4.  **Working Days**, which by default are days other than Saturdays, Sundays, and days when commercial banks in the capital of the country of _Developer's Address_ typically stay closed. The singular is **Working Day**.
...
_Client_ agrees to increase the amount of each payment to _Developer_ so that _Developer_ receives the full amount owed, net of any costs of the _Payment Method_.
...
2.  The _Technical Representative_ must send the _Developer_ written instructions for the work while there is still _Time Left_ and _Unused Hours_ to do the work.
...
_Developer_ guarantees that they will have all the rights needed to license work they and create and use to do [Public Licenses](#Public_Licenses). _Client_ agrees that _Developer_ may rely on written statements by authors of software, copies of and references to specific public licenses in "README", "LICENSE", source code, and similar files, and licensing metadata in "package.json" and similar, standardized metadata files, to determine their rights for the work of others. If _Developer_ breaches this guarantee, _Client_ may disclose that _Developer_ did so.
